#!/bin/bash # run_check.sh # Run this script from the top-level lme4 source directory

# 1. Install all required dependencies in the user library.
# Adjust the list below if your DESCRIPTION file indicates additional dependencies.
...
# 2. Build the source tarball from the current directory.
echo "Building the source tarball..."
R CMD build .

# 3. Identify the newly built tarball (assumes package name is lme4).
...
# 4. Perform an R CMD check on the tarball.
